...
const branchRegex = /^(feature|fix|hotfix)\/[A-Z]+-\d+-[a-z0-9]+(?:-[a-z0-9]+)*$|^chore\/[a-z0-9]+(?:-[a-z0-9]+)*$/;
...
if (!branchRegex.test(branch)) {
  ...
  console.error('- feature/CLIK-142-auth-navigation');
  console.error('- fix/CLIK-201-crash-on-start');
  console.error('- hotfix/CLIK-301-crash-on-launch');
